...
const fallbackExhibitors=['GOL TÊXTIL','DISTRIBUIDOR DE CAMISETAS','ECOMMERCE VERSO','PEDROSO TÊXTIL',"BETINI'S",'FÊNIX TÊXTIL','TW PRINT','MARGIS TRANSPORTES','MOURA MALHAS','FINAL Z','YGUAÇU MÁQUINAS','HR TÊXTIL','MECOLOUR','MAQCENTER COSTURA','BM DO BRASIL','MAGNA TECH','CONTÁBIL ZANONE','ZANONE CURSOS','ZANONE MALHAS'];
...
export default async function Home(){
 const [exRows,scheduleRows,articleRows,settingsRows]=await Promise.all([
  publicQuery('exhibitors?select=name,logo_url,website_url,stand&active=eq.true&name=not.ilike.*stellar*&order=sort_order.asc,name.asc'),
 ...
 return <main className="v3site">
   ...
   <TextileJourneyVideo/>
   <section id="expositores" className="v3section logoSection"><div className="v3wrap"><div className="v3sectionHead"><span>02 / ÁREA DE NEGÓCIOS</span><h2>MARCAS QUE JÁ<br/><em>ESTÃO NA TRAMA.</em></h2><p>19 empresas confirmadas. Uma amostra real da cadeia que vai se encontrar em Franca.</p></div><LogoCarousel/></div></section>
   ...
   <section className="v3section" style={{paddingTop:0}}><div className="v3wrap"><div style={{position:'relative',overflow:'hidden',borderRadius:'24px',border:'1px solid rgba(255,255,255,.14)',background:'#020617',boxShadow:'0 28px 90px rgba(0,0,0,.35)'}}><video controls playsInline preload="metadata" style={{display:'block',width:'100%',height:'auto',aspectRatio:'16 / 9',objectFit:'cover',background:'#020617'}}><source src="/media/videos/vt-franca-textil.mp4" type="video/mp4"/>Seu navegador não suporta vídeo HTML5.</video></div></div></section>
