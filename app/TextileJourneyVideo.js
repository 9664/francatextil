...
const SCENES=[
  {kicker:'01 / ORIGEM',title:'TUDO COMEÇA NO FIO.',copy:'É aqui que a cadeia começa: matéria-prima, textura e possibilidade.',source:{type:'b64',url:'/journey-hires/01-fio.b64'},focus:'58% 50%'},
  ...
  {kicker:'03 / INDÚSTRIA',title:'A PRODUÇÃO GANHA RITMO.',copy:'Máquinas, processos e conhecimento colocam a indústria em movimento.',source:{type:'url',url:'/scene3.webp'},focus:'58% 52%'},
  {kicker:'04 / PRODUTO',title:'DO PROCESSO NASCE VALOR.',copy:'A peça pronta reúne design, produção, marca e identidade.',source:{type:'url',url:'/scene4.webp'},focus:'50% 52%'},
  {kicker:'05 / MERCADO',title:'DO FIO AO CLIQUE.',copy:'Toda a cadeia no mesmo movimento — do produto ao mercado.',source:{type:'b64',url:'/journey-hires/05-caixa-fechada.b64'},focus:'58% 50%'}
];
...
async function materialize(source){
  ...
  const r=await fetch(source.url,{cache:'force-cache'});
  if(!r.ok)throw new Error(source.url);
...
export default function TextileJourneyVideo(){
  ...
  useEffect(()=>{
    ...
    Promise.all(SCENES.map(s=>materialize(s.source)))
      .then(urls=>{
        if(!alive)return;
        urlsRef.current=urls.filter(u=>u.startsWith('blob:'));
        setAssets(urls);
        setStatus('ready');
      })
      .catch(err=>{console.error('Journey stills failed',err);if(alive)setStatus('error')});
  ...
  return <section ref={sectionRef} id="do-fio-ao-clique" className="cinematicJourney stillJourney">
    <div className="cinematicSticky">
      <div className="stillStack" aria-hidden="true">
        {SCENES.map((s,i)=>{
          const d=Math.abs(stage-i);
          const opacity=clamp(1-d*1.55);
          const local=clamp(stage-i+1,0,1);
          const scale=1.015+(local*.075);
          return <img key={i} src={assets[i]||s.source.type==='url'?assets[i]||s.source.url:undefined} className="journeyStill" alt="" style={{opacity,transform:`scale(${scale})`,objectPosition:s.focus}}/>;
        })}
      </div>
      <div className="stillVignette" aria-hidden="true"/>
      <div className="cinematicCopy" key={active}>
        <span>{c.kicker}</span><h2>{c.title}</h2><p>{c.copy}</p>
      </div>
      ...
      {status==='error'&&<div className="cinematicStatus error">EXPERIÊNCIA VISUAL EM MODO DE SEGURANÇA</div>}
    </div>
  </section>;
}
